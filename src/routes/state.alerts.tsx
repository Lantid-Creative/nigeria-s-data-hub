...
export const Route = createFileRoute("/state/alerts")({ component: () => (
  <div className="space-y-6"><SectionHeader title="Notifications" description="Personalised alerts for your state" />
  <Card className="shadow-soft"><CardContent className="divide-y p-0">
    {ALERTS.map((a, i) => (
      <div key={i} className="flex items-center gap-4 p-4">
        <span className={`h-2 w-2 rounded-full ${a.level === "high" ? "bg-destructive" : a.level === "medium" ? "bg-gold" : "bg-[color:var(--info)]"}`} />
